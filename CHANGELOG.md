...
- **data-plane-api Kubernetes Job runner is now the default** (`dataPlane.components.api.k8sJobs.enabled: true`). The data-plane-api Deployment spawns evaluation workloads as ephemeral Jobs in the release namespace instead of running them as FastAPI background tasks — keeping the API pod responsive and bounding per-evaluation resource usage. A new namespaced `ServiceAccount data-plane-api`, `Role data-plane-job-creator`, and matching `RoleBinding` are rendered automatically; the API Deployment binds to the new SA and receives `K8S_JOBS_ENABLED`, `K8S_JOB_SECRETS_MODE`, `K8S_JOBS_NAMESPACE`, `K8S_JOB_IMAGE`, `K8S_JOB_SERVICE_ACCOUNT` plus tunable resource/TTL/concurrency env vars. **Requires `data-plane-api >= v1.25.0`**. To opt out on older images, set `dataPlane.components.api.k8sJobs.enabled: false`.
- **`secretsMode: inherit` is the default** for the new Job runner. The API process forwards its own env vars into each Job pod's `env:` block at job-creation time, so Jobs work identically on Helm (Kubernetes Secrets) and Flux + Secret Manager (CSI) without any Secret object, SecretProviderClass, or extra mount. Operators on the legacy CSI flow can set `dataPlane.components.api.k8sJobs.secretsMode: csi` to keep prior behaviour.
