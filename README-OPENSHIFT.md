...
- If not using cert-manager, Kubernetes Ingress uses the chart's shared self-signed `kubernetes.io/tls` secret by default unless you override `tls.secretName`
...
**All required secrets are auto-generated by default** (`global.autoGenerateSecrets: true`). You only need to create the image pull secret (`gcr-secret`) and set your OpenShift domain. Optionally copy `values-openshift.yaml` to customize further.

When you use **OpenShift Routes**, the shared ingress TLS secret is not involved. When you use **Kubernetes Ingress** on OpenShift, the chart creates a shared self-signed TLS secret by default, the bundled ingress defaults enable HTTPS automatically, and any empty `tls.secretName` uses that shared secret.

**From OCI (Artifact Registry) — no clone required:**

```bash
oc create namespace neuraltrust
# Create image pull secret (required for private NeuralTrust/TrustGate images)
...
Replace `VERSION` with a [release version](https://github.com/NeuralTrust/neuraltrust-platform/releases) (e.g. `1.2.7`). For full OpenShift options, copy `values-openshift.yaml`.
...
# 2. Create image pull secret (required for private images)

# 3. (Optional) Copy values-openshift.yaml to customize domains, resources, etc.
...
# 5. Deploy (secrets auto-generated)
...
With `global.autoGenerateSecrets: true` (the default), the following secrets are **automatically generated** by the chart on first install and preserved on upgrades. **No manual creation is required:**

| Secret Name | Key | Auto-Generated | Description |
|------------|-----|:--------------:|-------------|
...
> **Note:** `SERVER_SECRET_KEY` and `TRUSTGATE_JWT_SECRET` are always synchronized to the same value.

### Pre-Generated Secrets (When `preserveExistingSecrets: true`)

When using pre-generated secrets (`global.autoGenerateSecrets: false`, `global.preserveExistingSecrets: true`), all secrets must be created in the target namespace before deployment.

### Data Plane Secrets

| Secret Name | Key | Required | Description |
|------------|-----|----------|-------------|
| `data-plane-jwt-secret` | `DATA_PLANE_JWT_SECRET` | **Auto-generated** | JWT secret for Data Plane API authentication |
...
| Secret Name | Key | Required | Description |
|------------|-----|----------|-------------|
| `control-plane-secrets` | `CONTROL_PLANE_JWT_SECRET` | **Auto-generated** | JWT secret for Control Plane API authentication |
...
| `control-plane-secrets` | `TRUSTGATE_JWT_SECRET` | **Auto-generated** | JWT secret for TrustGate integration (synced with `SERVER_SECRET_KEY`) |
...
| Secret Name | Key | Required | Description |
|------------|-----|----------|-------------|
| `postgresql-secrets` | `POSTGRES_HOST` | **Yes** (if pre-generating) | PostgreSQL database hostname |
| `postgresql-secrets` | `POSTGRES_PORT` | **Yes** (if pre-generating) | PostgreSQL database port (default: 5432) |
| `postgresql-secrets` | `POSTGRES_USER` | **Yes** (if pre-generating) | PostgreSQL database username |
| `postgresql-secrets` | `POSTGRES_PASSWORD` | **Auto-generated** | PostgreSQL database password |
| `postgresql-secrets` | `POSTGRES_DB` | **Yes** (if pre-generating) | PostgreSQL database name |
| `postgresql-secrets` | `DATABASE_URL` | **Yes** (if pre-generating) | PostgreSQL connection URL (URL-encoded) |
| `postgresql-secrets` | `POSTGRES_PRISMA_URL` | **Yes** (if pre-generating) | Prisma-compatible PostgreSQL connection URL |

### Infrastructure Secrets

| Secret Name | Key | Required | Description |
|------------|-----|----------|-------------|
| `clickhouse` | `admin-password` | **Auto-generated** (if deploying ClickHouse) | ClickHouse admin password for infrastructure ClickHouse instance |
...
| Secret Name | Key | Required | Description |
|------------|-----|----------|-------------|
| `trustgate-secrets` | `SERVER_SECRET_KEY` | **Auto-generated** | TrustGate server secret key (synced with `TRUSTGATE_JWT_SECRET`) |
...
### Step 2: Create Image Pull Secret (if using private images)

With `global.autoGenerateSecrets: true` (the default), all platform secrets are auto-generated. You only need to create the image pull secret:

- `gcr-secret` (required for private NeuralTrust/TrustGate images)

**Note:** If using `global.preserveExistingSecrets: true`, you must also create all platform secrets manually. See [Creating Secrets](#creating-secrets) for details.
...
Create or modify your values file for OpenShift. Secrets are auto-generated if left empty:
