...
**Chart source:** We recommend installing from a [GitHub Release](https://github.com/NeuralTrust/neuraltrust-platform/releases) or [Artifact Registry](README.md#releases-and-installing-the-chart) (OCI), not from the `main` branch. The examples below use `.` for a local chart; when using a release, replace `.` with the path to the `.tgz` or the OCI URL and add `--version VERSION`. See [Releases and Installing the Chart](README.md#releases-and-installing-the-chart) in the main README.
...
### Step 3: Update Helm Dependencies (if using a local clone)

```bash
helm dependency update
```

Skip this step when installing from OCI or a release tarball.
...
- [Main README](./README.md) - General deployment guide, [releases and install options](./README.md#releases-and-installing-the-chart)
